// Используем глобально доступные объекты gsap и ScrollTrigger
// которые загружаются через CDN в index.html
...
export function initAnimations() {
  ...
  // Set defaults for all animations
  gsap.defaults({
    ease: 'power2.out',
    duration: 0.8,
    overwrite: true
  ...
  setTimeout(() => {
    console.log('Initializing animations with delay');
...
function animateHeader() {
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => console.log('Header scroll effect activated')
  });
}
...
function animateHero() {
  ...
  if (!heroTitle || !heroDescription || !heroCta || !heroImage) {
    console.warn('One or more hero elements not found');
    return;
  }
    ...
    sectionTitles.forEach(title => {
      ...
      gsap.to(title, {
        scrollTrigger: {
          ...
          onEnter: () => console.log('Section title animation triggered')
        },
        opacity: 1,
        y: 0,
        duration: 0.8,
...
function animateProducts() {
  ...
  if (productTabs.length) {
    ...
    gsap.to(productTabs, {
      scrollTrigger: {
        ...
        start: 'top 80%',
        onEnter: () => console.log('Product tabs animation triggered')
      },
      opacity: 1,
      y: 0,
      stagger: 0.1,
  ...
  if (products.length) {
    // Animate the initially active product
    ...
    if (activeProduct) {
      ...
      if (productImage && productInfo) {
        ...
        const productTl = gsap.timeline({
          ...
          onComplete: () => console.log('Active product animation completed')
        });
  ...
  // Handle tab clicks to animate new active product
...
function animateAdvantages() {
  ...
  if (!advantages.length) {
    console.warn('No advantage elements found for animations');
    return;
  }
  
  console.log(`Found ${advantages.length} advantage elements for animation`);
  
  // Set initial states
  gsap.set(advantages, { opacity: 0, y: 50 });
  
  // Create the main animation for advantages
  gsap.to(advantages, {
    scrollTrigger: {
      trigger: advantages[0].closest('.advantages'),
      start: 'top 80%',
      onEnter: () => console.log('Advantages animation triggered')
    },
    y: 0,
    opacity: 1,
    stagger: 0.15,
    ...
    ease: 'back.out(1.2)',
    clearProps: "y",
    onComplete: () => console.log('Advantages animation completed')
  });
  
  // Animate each advantage icon separately
  advantages.forEach((advantage, index) => {
    ...
    if (icon) {
      ...
      // Floating animation with randomized timing
...
function animateClients() {
  ...
  if (!clients.length) {
    console.warn('No client elements found');
    return;
  }
  ...
  gsap.to(clients, {
    scrollTrigger: {
      ...
      start: 'top 80%',
      onEnter: () => console.log('Clients animation triggered')
    },
    scale: 1,
    opacity: 1,
    stagger: 0.1,
...
function animateFooter() {
  ...
  if (!footerSections.length) {
    console.warn('No footer sections found');
    return;
  }
...
function init3DEffects() {
  ...
  // Add floating animation to advantage icons
